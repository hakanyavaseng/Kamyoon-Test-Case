...
public class ReadRepository<T> : IReadRepository<T> where T : CreationAuditedEntity<Guid>
{
        ...
        int pageSize = 3)
    {
        ...
        if (predicate is not null)
            queryable = queryable.Where(predicate); //Where clause.
        if (orderBy is not null)
            return await orderBy(queryable).Skip((currentPage - 1) * pageSize).Take(pageSize)
                .ToListAsync(); //Paging and order by
